...
const Post = () => {
  ...
  const handleReportPost = async (e) => {
    ...
    await reportedPostMutation.mutate({
      postID: id,
      ...
      {post && postAuthor && (
        <div className="border border-gray-300 p-4 rounded-lg shadow-lg">
          <div className="flex items-center mb-4">
            ...
            />
            <div>
              <h2 className="text-xl font-bold text-black">{postAuthor.name}</h2>
              <p className="text-gray-600">@{postAuthor.email.split("@")[0]}</p>
            </div>
          </div>
          <h3 className="text-2xl font-semibold mb-2 text-black">{post.title}</h3>
            ...
            <div className="flex flex-row justify-start items-center gap-4">
              <button
                className={`flex items-center text-gray-500 ${
                  myVotes.find((vote) => vote.postId === post._id)?.voteType === "up"
                    ? "text-blue-500"
                    : ""
                }`}
              ...
              >
                ...
                {
                  votes.filter((vote) => vote.postId === post._id && vote.voteType === "up")
                    .length
                }
              </button>
              <button
                className={`flex items-center text-gray-500 ${
                  myVotes.find((vote) => vote.postId === post._id)?.voteType === "down"
                    ? "text-red-500"
                    : ""
                }`}
              ...
              >
                ...
                {
                  votes.filter((vote) => vote.postId === post._id && vote.voteType === "down")
                    .length
                }
              </button>
            </div>
              ...
              {user && user.email !== post.owner && (
                ...
                <button
                  ...
                  onClick={() => {
                    const res = postReports?.find(
                      (report) => report.postID === post?._id && report.reporterEmail === user?.email
                    );
                    if (user.length === 0) {
                      Swal.fire({
                        icon: "error",
                ...
                <dialog id="my_modal_2" className="modal">
                  <div className={`modal-box bg-white ${reportModal}`}>
                    <h3 className="font-bold text-lg text-black">Report This Post</h3>
                    <hr />
                    <div className="mt-6 flex justify-center items-center">
                      ...
                      >
                        ...
                        >
                          ...
                          <option value="Contains nudity">Contains nudity</option>
                          <option value="Violating privacy">Violating privacy</option>
          ...
          <div>
            ...
            <div className="max-h-96 overflow-y-auto">
              {comments
                ...
                .map((comment) => {
                  const commentUser = users.find((user) => user.email === comment.email);
                  return (
                    <div key={comment._id} className="border border-gray-300 p-2 my-2 rounded-md relative">
                      ...
                      <div className="flex items-center mb-2">
                        ...
                        />
                        <span className="text-sm font-semibold text-black">{commentUser.name}</span>
                      </div>
                      <p className="text-black">{comment.comment}</p>
                      <span className="text-gray-500 text-xs">{timeSince(comment.date)}</span>
                    </div>
                  );
                })}
            </div>
            <form onSubmit={handleAddComment} className="mt-4 flex flex-col">
              ...
              />
              <button className="mt-2 p-2 bg-blue-500 text-white rounded-lg" type="submit">
                Post Comment
              </button>
            </form>
          </div>
